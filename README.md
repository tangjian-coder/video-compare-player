...
- **十字准星**：`C` 在两侧**窗格中心**显示一条 **1px 红色十字线**（mpv OSD 渲染），缩放/平移/旋转时**纹丝不动**——这是对比两侧运动幅度的稳定参照物，会被录进录屏
- 拖放加载，支持命令行直接传两个视频
...
`Space` 同步播放/暂停 · `←`/`→` 双侧逐帧 · `↑`/`↓` 变速 · `Home`/`End` 跳对齐区间首尾 · `S` 一键对齐 · `R` 清除同步 · `F9` 录屏开始/停止 · `C` 十字准星 · `Ctrl+O` 打开视频
